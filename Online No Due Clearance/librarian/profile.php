...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['profile_image']) && $_FILES['profile_image']['error'] == 0) {
        ...
        if (in_array(strtolower($filetype), $allowed)) {
            $new_filename = "librarian_" . $librarian_id . "_" . time() . "." . $filetype;
            $upload_path = "../images/" . $new_filename;
            
            if (move_uploaded_file($_FILES['profile_image']['tmp_name'], $upload_path)) {
                // Update profile with new image
                $update_query = "UPDATE librarian SET name = '$name', email = '$email', mobile = '$mobile', profile_image = '$new_filename' WHERE librarian_id = $librarian_id";
            }
        }
    } else {
        // Update profile without changing image
        $update_query = "UPDATE librarian SET name = '$name', email = '$email', mobile = '$mobile' WHERE librarian_id = $librarian_id";
    }
...
<head>
    ...
    <title>Librarian Profile - College Management System</title>
                ...
                <div class="col-md-4">
                    <div class="card">
                        ...
                        <div class="card-body text-center">
                            <img src="../images/<?php echo $librarian['profile_image']; ?>" alt="Profile Image" class="profile-image mb-3">
                        ...
                        <div class="card-body">
                            <form method="post" enctype="multipart/form-data">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="name" name="name" class="form-control" value="<?php echo $librarian['name']; ?>" required>
                                </div>
                                
                                <div class="form-group">
                                    <label for="email">Email</label>
                                    <input type="email" id="email" name="email" class="form-control" value="<?php echo $librarian['email']; ?>" required>
                                </div>
                                
                                <div class="form-group">
                                    <label for="mobile">Mobile Number</label>
                                    <input type="text" id="mobile" name="mobile" class="form-control" value="<?php echo $librarian['mobile']; ?>" required>
                                </div>
