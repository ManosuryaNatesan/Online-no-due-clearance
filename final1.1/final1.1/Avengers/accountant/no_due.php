...
$query = "SELECT r.*, s.name as student_name, s.register_number, s.department, s.year_of_study
          ...
          WHERE r.faculty_approval = 'Approved' AND r.librarian_approval = 'Approved' AND r.accountant_approval = 'Pending'
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['approve']) || isset($_POST['reject'])) {
        ...
        // Check if student has any outstanding fees or fines
        $check_query = "SELECT COUNT(*) as unpaid_fees, SUM(fine_amount) as total_fine
                       FROM fees 
        ...
        if (isset($_POST['approve'])) {
            if ($check_data['unpaid_fees'] > 0 || $check_data['total_fine'] > 0) {
                $error_message = "Cannot approve. Student has outstanding fees or fines.";
            } else {
                $status = 'Approved';
        ...
        if (!isset($error_message)) {
            $update_query = "UPDATE no_due_requests 
                           SET accountant_approval = '$status', 
                               accountant_remarks = '$remarks',
                               accountant_approval_date = NOW()";
            
            // If all approvals are completed and this is approved, update the final status
            if ($status == 'Approved') {
                $update_query .= ", final_status = CASE 
                                   WHEN faculty_approval = 'Approved' 
                                   AND librarian_approval = 'Approved' 
                                   AND accountant_approval = 'Approved' 
                                   AND hod_approval = 'Approved' 
                                   THEN 'Approved' 
                                   ELSE final_status END";
            } else if ($status == 'Rejected') {
                $update_query .= ", final_status = 'Rejected'";
            }
...
<body>
    <div class="dashboard">
        ...
        <div class="main-content">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <p>As an accountant, you're responsible for approving no due requests from students who have cleared all fee dues. Before approving, please ensure:</p>
                    <ul>
                        <li>All tuition fees have been paid by the student</li>
                        <li>All transport fees have been cleared</li>
                        <li>All exam fees have been settled</li>
                            ...
                            <table class="table" id="requests-table">
                                ...
                                <tbody>
                                    <?php while ($request = mysqli_fetch_assoc($all_requests_result)): ?>
                                        <tr data-status="<?php echo $request['accountant_approval']; ?>">
                                            <td><?php echo $request['student_name']; ?></td>
                                            <td><?php echo $request['register_number']; ?></td>
                                            <td><?php echo $request['department']; ?></td>
                                            <td><?php echo $request['year_of_study']; ?></td>
    ...
    <script>
        ...
        function openApprovalForm(requestId, studentId, studentName) {
            ...
            fetch('get_student_fee_status.php?id=' + studentId)
                .then(response => response.json())
                .then(data => {
                    let statusHTML = '<h4>Fee Status</h4>';
                    
                    if (data.unpaid_fees > 0) {
                        statusHTML += '<div class="alert alert-danger">Student has ' + data.unpaid_fees + ' unpaid or partially paid fee records.</div>';
                    } else {
                        statusHTML += '<div class="alert alert-success">All fees have been paid.</div>';
                    }
                ...
                .catch(error => {
                    ...
                    document.getElementById("student-status").innerHTML = '<div class="alert alert-danger">Error fetching student fee status</div>';
                });
